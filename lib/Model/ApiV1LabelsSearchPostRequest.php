...
/**
 * ApiV1LabelsSearchPostRequest Class Doc Comment
 *
 * @category Class
 * @package  Boxnow
 * @author   OpenAPI Generator team
 ...
 */
class ApiV1LabelsSearchPostRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = '_api_v1_labels_search_post_request';
    ...
    public const PAPER_SIZE_A6 = 'A6';
    public const PER_PAGE_NUMBER_1 = 1;
    public const PER_PAGE_NUMBER_2 = 2;
    public const PER_PAGE_NUMBER_4 = 4;
     ...
     * @return string[]
     */
    public function getPerPageAllowableValues()
    {
        return [
            self::PER_PAGE_NUMBER_1,
            self::PER_PAGE_NUMBER_2,
            self::PER_PAGE_NUMBER_4,
        ];
    }
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('paper_size', $data ?? [], 'A6');
        $this->setIfExists('per_page', $data ?? [], self::PER_PAGE_NUMBER_1);
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['paper_size']) && !in_array($this->container['paper_size'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'paper_size', must be one of '%s'",
                $this->container['paper_size'],
                implode("', '", $allowedValues)
            );
        }

        $allowedValues = $this->getPerPageAllowableValues();
        if (!is_null($this->container['per_page']) && !in_array($this->container['per_page'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'per_page', must be one of '%s'",
     ...
     * Sets parcel_ids
     *
     * @param string[]|null $parcel_ids Response will contain only labels for these parcels (if defined).
     *
     * @return self
     */
     ...
     * Sets order_numbers
     *
     * @param string[]|null $order_numbers Response will contain only labels for these orders (if defined).
     *
     * @return self
     */
     ...
     * Sets paper_size
     *
     * @param string|null $paper_size Output PDF sheet size
     *
     * @return self
     */
    public function setPaperSize($paper_size)
    {
        ...
        $allowedValues = $this->getPaperSizeAllowableValues();
        if (!in_array($paper_size, $allowedValues, true)) {
     ...
     * Sets per_page
     *
     * @param float|null $per_page Number of labels per sheet.
     *
     * @return self
     */
    public function setPerPage($per_page)
    {
        ...
        $allowedValues = $this->getPerPageAllowableValues();
        if (!in_array($per_page, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'per_page', must be one of '%s'",
                    $per_page,
